      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div className="overflow-x-auto">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mt-6">
            <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Employee Summary</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {employees.map((employee) => {
                const employeeEntries = analytics.entries.filter((entry: any) => 
                  (entry.employeeId?.id || entry.employeeId) === employee.id && entry.hoursWorked
                );
                const totalHours = employeeEntries.reduce((sum, entry) => sum + (entry.hoursWorked || 0), 0);
                ...
                return (
                  <div key={employee.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                    ...
                    <div className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                      <p>Total Hours: {totalHours.toFixed(1)}h</p>
                      <p>Total Shifts: {employeeEntries.length}</p>
                      <p>Role: {employee.isAdmin ? 'Admin' : 'Employee'}</p>
                    </div>
                  </div>
                );
              })}
